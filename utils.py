...
def email_api_key_request(logger, req, minutes_expiry):
    url = url_for("paydb.api_key_confirm", token=req.token, secret=req.key, _external=True)
...
def email_stash_request(logger, email, req, minutes_expiry):
    url = url_for("stash_bp.stash_save_confirm", token=req.token, secret=req.secret, _external=True)
    msg = f"You have a pending stash request waiting!<br/><br/>Confirm your stash <a href='{url}'>here</a><br/><br/>Confirm within {minutes_expiry} minutes"
    send_email(logger, "Confirm your stash request", msg, email)

def email_stash_loadrequest(logger, email, req, minutes_expiry):
    url = url_for("stash_bp.stash_load_confirm", token=req.token, secret=req.secret, _external=True)
    msg = f"You have a pending stash load request waiting!<br/><br/>Confirm your stash load <a href='{url}'>here</a><br/><br/>Confirm within {minutes_expiry} minutes"
    send_email(logger, "Confirm your stash load request", msg, email)
